...
export const PlateViewer = forwardRef<PlateHandle, { file: File | null; bed?: number; bedY?: number; onObject?: (info: ObjInfo | null) => void; onState?: (s: PlateState) => void; onContextMenu?: (x: number, y: number, index: number) => void; slotColors?: string[]; showOrder?: boolean; clearance?: number; onMovePlate?: (srcPlate: number, srcUuid: string, targetPlate: number) => void }>(function PlateViewer({ file, bed = 256, bedY: bedYProp, onObject, onState, onContextMenu, slotColors, showOrder, clearance = 0, onMovePlate }, ref) {
  ...
  useEffect(() => {
    ...
    // ── "All plates" overview: drag an object onto another plate's cell ──
    let ovDrag: THREE.Mesh | null = null; let ovSrcPlate = -1; let ovUuid = '';
    const ovPlane = new THREE.Plane(new THREE.Vector3(0, 0, 1), 0);
    const ovHitPt = new THREE.Vector3();
    const planePoint = (ev: PointerEvent) => {
      const r = renderer.domElement.getBoundingClientRect();
      const nx = ((ev.clientX - r.left) / r.width) * 2 - 1;
      const ny = -((ev.clientY - r.top) / r.height) * 2 + 1;
      ray.setFromCamera(new THREE.Vector2(nx, ny), camera);
      return ray.ray.intersectPlane(ovPlane, ovHitPt) ? ovHitPt.clone() : null;
    };

    const onPointerMove = (ev: PointerEvent) => {
      if (ovDrag) { const p = planePoint(ev); if (p) { ovDrag.position.x = p.x; ovDrag.position.y = p.y; } return; }
    ...
    const onPointerDown = (ev: PointerEvent) => {
      ...
      const c = ctx.current;
      if (c?.overview) {
        // Pick an object clone in the overview to start dragging it.
        const r = renderer.domElement.getBoundingClientRect();
        const nx = ((ev.clientX - r.left) / r.width) * 2 - 1;
        const ny = -((ev.clientY - r.top) / r.height) * 2 + 1;
        ray.setFromCamera(new THREE.Vector2(nx, ny), camera);
        const meshes = c.overview.children.filter((o) => (o as THREE.Mesh).isMesh && o.userData.srcUuid) as THREE.Mesh[];
        const hit = ray.intersectObjects(meshes, false)[0];
        if (hit) { ovDrag = hit.object as THREE.Mesh; ovSrcPlate = ovDrag.userData.srcPlate; ovUuid = ovDrag.userData.srcUuid; orbit.enabled = false; }
        return;
      }
    ...
    const onPointerUp = (ev: PointerEvent) => {
      if (ovDrag) {
        const c = ctx.current;
        const cells = (c?.overview?.userData.cells as { cx: number; cy: number }[] | undefined) || [];
        let best = -1, bestD = Infinity;
        cells.forEach((cl, i) => { const d = (cl.cx - ovDrag!.position.x) ** 2 + (cl.cy - ovDrag!.position.y) ** 2; if (d < bestD) { bestD = d; best = i; } });
        orbit.enabled = true;
        const src = ovSrcPlate, uuid = ovUuid; ovDrag = null; ovSrcPlate = -1; ovUuid = '';
        if (best >= 0 && best !== src) onMoveRef.current?.(src, uuid, best);
        else onMoveRef.current?.(src, uuid, src);   // snap back: caller rebuilds the overview
        return;
      }
    ...
    const syncOverview = () => {
      const cells = (c.overview?.userData.cells as { cx: number; cy: number; label: string }[] | undefined) || [];
  ...
  async function loadFileBuffer(f: File) {
    ...
    try {
      ...
      if (lower.endsWith('.3mf')) {
        // A 3MF's <build> lists one <item> per placed OBJECT; the ThreeMFLoader
        // adds one child per build item. Each build item is a separate,
        // individually-movable object; the components/meshes inside one item are
        // that object's coloured parts (kept merged). So split by build item.
        const obj = new ThreeMFLoader().parse(buf);
        obj.updateMatrixWorld(true);
        // Real per-part colours from the Bambu/Orca config (extruder → filament
        // colour), aligned with the flattened mesh order.
        const cfg = parse3mfColors(buf);
        let totalMeshes = 0; obj.traverse((o) => { if ((o as THREE.Mesh).isMesh) totalMeshes++; });
        if (!totalMeshes) return;
        const useCfg = !!cfg && cfg.colors.length > 0 && cfg.extruders.length === totalMeshes;
        ...
        // One movable object per build item (direct children of the loaded group).
        const items = obj.children.length ? [...obj.children] : [obj];
        const perItem = items.map((it) => { const ms: THREE.Mesh[] = []; it.traverse((o) => { if ((o as THREE.Mesh).isMesh) ms.push(o as THREE.Mesh); }); return ms; });
        const nObjects = perItem.filter((ms) => ms.length).length;
        let gIdx = 0, ci = 0;
        for (const ms of perItem) {
          if (!ms.length) continue;
          const geoms = ms.map((m) => buildGeom(m, gIdx++));
          const merged = mergeGeometries(geoms, false);
          if (!merged) continue;
          merged.computeBoundingBox();
          const bb = merged.boundingBox!;
          const cx = (bb.min.x + bb.max.x) / 2, cy = (bb.min.y + bb.max.y) / 2, cz = (bb.min.z + bb.max.z) / 2;
          // Per-extruder split for a multi-colour object (aligned to the shared frame).
          ...
          addMesh(merged, nObjects > 1 ? `${base} (${++ci})` : base, true, materials);
        }
  ...
  useImperativeHandle(ref, () => ({
    ...
    setAllPlatesView: (groups, activeIdx = 0) => {
      ...
      groups.forEach((g, idx) => {
        ...
        cells.push({ cx: ox, cy: oy, label: g.name || `Plate ${idx + 1}` });
        ...
        // Clones of the plate's objects, translated into the cell. Tagged so a
        // drag can move the real object to another plate.
        for (const m of g.meshes) {
          ...
          cm.userData.srcPlate = idx; cm.userData.srcUuid = m.uuid; cm.userData.cellOx = ox; cm.userData.cellOy = oy;
          grp.add(cm);
        }
      });
      grp.userData.cells = cells;
      c.scene.add(grp); c.overview = grp;
      // Frame the whole grid with a little margin.
      const span = Math.max(cols, rows) * cell * 1.25;
      c.camera.position.set(0, -span * 1.05, span * 0.95);
