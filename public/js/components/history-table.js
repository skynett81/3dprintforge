        ...
        for (const row of pageItems) {
          ...
          h += `<div class="ph-card" data-status="${row.status}" data-review="${row.review_status || ''}" data-id="${row.id}" onclick="showHistoryDetail(${row.id})">
            ...
            <div class="ph-card-info">
              <div class="ph-card-name" title="${esc(displayName)}">${(row.model_url || cloud?.designId) ? `<a href="${esc(row.model_url || 'https://makerworld.com/en/models/' + cloud.designId)}" target="_blank" rel="noopener" class="ph-model-link-icon" onclick="event.stopPropagation()" title="Open model"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg></a> ` : ''}${colorDots(filamentColors(row))}${esc(displayName)}</div>
