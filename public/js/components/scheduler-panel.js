...
(function() {
      ...
      return `<div style="display:flex;align-items:center;gap:10px;padding:10px;border-radius:var(--radius-sm);cursor:pointer;transition:background 0.1s;border:1px solid var(--border-subtle);margin-bottom:6px" onclick="_schedEdit('${ev.id}')" onmouseenter="this.style.background='var(--bg-tertiary)'" onmouseleave="this.style.background=''">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:0.7rem;color:var(--text-muted);display:flex;gap:8px;align-items:center;margin-top:2px;flex-wrap:wrap">
            ...
            ${colorSwatch}
            ${filamentInfo}
            ${ev.review_status === 'approved' ? '<span style="color: var(--accent-green-text)" title="Approved">&#10003;</span>' : ev.review_status === 'rejected' ? '<span style="color:var(--accent-red)" title="Rejected">&#10007;</span>' : ev.review_status === 'partial' ? '<span style="color:var(--accent-orange)" title="Partial">&#9680;</span>' : (ev._fromHistory && !ev._fromLive ? '<span style="color:var(--text-muted);opacity:0.5" title="Not reviewed">?</span>' : '')}
          </div>
        </div>
      </div>`;
    }).join('');
  ...
  function _showHistoryDetail(ev) {
    ...
    let thumbHtml = '';
    if (ev._historyId) {
      const safeTitle = _esc(ev.title || ev.filename || '').replace(/\\/g, '\\\\').replace(/'/g, "\\'");
    ...
    if (ev._fromHistory && ev._historyId) {
      ...
      const reviewBadge = rs === 'approved' ? '<span style="color: var(--accent-green-text);font-weight:600">&#10003; Approved</span>'
