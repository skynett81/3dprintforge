    ...
    if (_printerSubTab === 'list') {
      ...
      if (printers.length === 0) {
        ...
        for (const pr of printers) {
          const state = window.printerState?.printers?.[pr.id];
        ...
        for (const pr of printers) {
          ...
          if (!infoEl) continue;
          const pState = window.printerState?.printers?.[pr.id];
  ...
  function _renderSystemSubContent() {
    ...
    if (_systemSubTab === 'updates') {
      ...
      h += `<div class="settings-card">
        ...
        ${brand('AnkerMake (Community Klipper)', '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10"/></svg>', iCheck, 'Via Moonraker — M5 series with community Klipper firmware (not stock AnkerMake firmware)')}
      ...
      h += `<div class="settings-card">
        ...
        <p class="text-muted" style="font-size:0.85rem;margin-bottom:0.75rem">Export all settings as a JSON file, or import from a previous export.</p>
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          <button class="form-btn form-btn-primary" data-ripple onclick="window._exportDashboardSettings()">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-2px;margin-right:4px"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>Export settings
          </button>
          <button class="form-btn form-btn-sm" data-ripple onclick="window._importDashboardSettings()">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-2px;margin-right:4px"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>Import settings
          </button>
        </div>
