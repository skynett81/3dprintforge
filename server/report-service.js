...
export async function sendReportEmail(report) {
  ...
  const subject = `3DPrintForge — ${report.period} Report (${report.from} → ${report.to})`;
...
<title>3DPrintForge — ${summary.period} Report</title>
<style>
...
<div class="container">
  ...
  <div class="footer">Generated by 3DPrintForge · ${new Date().toISOString().split('T')[0]}</div>
</div>
</body>
</html>`;
}
...
async function _sendHtmlEmail(conf) {
  ...
  return new Promise((resolve, reject) => {
    ...
    function processLine(line) {
      ...
      switch (step) {
        case 0: if (code === 220) { send(`EHLO 3dprintforge`); step = 1; } break;
        case 1:
