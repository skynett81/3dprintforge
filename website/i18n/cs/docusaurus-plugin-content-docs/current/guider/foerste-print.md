...
Když je tisk hotov, panel zobrazí zprávu o dokončení (a odešle upozornění, pokud máte nastavena [upozornění](./varsler-oppsett)).
...
- **Važte prázdné cívky** — zadejte počáteční hmotnost cívek pro přesný výpočet zbytku (viz [Správa filamentů](./filament-oppsett))
- **Nastavte upozornění Telegram** — dostávejte zprávu, když je tisk hotov, aniž byste čekali (viz [Upozornění](./varsler-oppsett))
- **Kontrolujte tiskovou desku** — čistá deska = lepší přilnavost. Otřete IPA (isopropanolem) mezi tisky
- **Používejte správnou desku** — viz [Výběr správné tiskové desky](./velge-rett-plate), co se hodí pro váš filament
