...
When the print is done, the dashboard shows a completion message (and sends a notification if you have set up [alerts](./varsler-oppsett)).
...
- **Weigh empty spools** — enter the starting weight on spools for accurate remaining estimates (see [Filament Inventory](./filament-oppsett))
- **Set up Telegram notifications** — get notified when the print is done without having to wait around (see [Notifications](./varsler-oppsett))
- **Check the build plate** — clean plate = better adhesion. Wipe with IPA (isopropyl alcohol) between prints
- **Use the right plate** — see [Choosing the right build plate](./velge-rett-plate) for what suits your filament
