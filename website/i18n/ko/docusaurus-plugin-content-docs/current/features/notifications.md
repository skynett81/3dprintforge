...
3DPrintForge는 다양한 채널을 통한 알림을 지원하여 집에 있든 외출 중이든 항상 프린터 상황을 파악할 수 있습니다.
...
5. 3DPrintForge에서: 토큰과 채팅 ID를 붙여넣고 **테스트** 클릭
...
4. 웹훅 URL 복사
5. 3DPrintForge에 URL 붙여넣고 **테스트** 클릭
...
4. 3DPrintForge에 둘 다 입력하고 **테스트** 클릭

### 웹훅(사용자 지정)

3DPrintForge는 JSON 페이로드와 함께 HTTP POST를 전송합니다:
...
:::info PWA
열린 탭 없이 백그라운드에서 푸시 알림을 받으려면 3DPrintForge를 PWA로 설치합니다. [PWA](../system/pwa)를 참조하세요.
:::
