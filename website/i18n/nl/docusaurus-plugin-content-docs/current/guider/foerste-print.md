...
Als de print klaar is, toont het dashboard een voltooiingsmelding (en stuurt een melding als je [meldingen](./varsler-oppsett) hebt ingesteld).
...
- **Weeg lege spoelen** — voer het startgewicht van spoelen in voor nauwkeurige restberekening (zie [Filamentbeheer](./filament-oppsett))
- **Stel Telegram-meldingen in** — ontvang een bericht als de print klaar is zonder te hoeven wachten (zie [Meldingen](./varsler-oppsett))
- **Controleer de bouwplaat** — schone plaat = betere hechting. Veeg af met IPA (isopropanol) tussen prints
- **Gebruik de juiste plaat** — zie [De juiste bouwplaat kiezen](./velge-rett-plate) voor wat past bij jouw filament
