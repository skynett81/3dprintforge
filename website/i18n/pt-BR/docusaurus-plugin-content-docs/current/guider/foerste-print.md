...
description: Guia passo a passo para iniciar sua primeira impressão 3D e monitorá-la no Bambu Dashboard
---

# Sua primeira impressão

Este guia vai te acompanhar por todo o processo — desde uma impressora conectada até uma impressão finalizada — com o Bambu Dashboard como centro de controle.
...
O Bambu Dashboard não inicia impressões diretamente — isso é tarefa do Bambu Studio ou MakerWorld. O painel assume assim que a impressão começa.
...
### Print Guard
O Bambu Dashboard tem um **Print Guard** movido por IA que detecta automaticamente falhas de espaguete e pode pausar a impressão. Ative em **Monitoramento → Print Guard**.

## Passo 5 — Após a impressão

Quando a impressão terminar, o painel exibe uma mensagem de conclusão (e envia uma notificação se você configurou [notificações](./varsler-oppsett)).
...
- **Pese os carretéis vazios** — insira o peso inicial dos carretéis para cálculo preciso do restante (veja [Gerenciamento de filamento](./filament-oppsett))
- **Configure notificações do Telegram** — receba uma mensagem quando a impressão terminar sem precisar ficar esperando (veja [Notificações](./varsler-oppsett))
- **Verifique a mesa** — mesa limpa = melhor aderência. Limpe com IPA (isopropanol) entre impressões
- **Use a placa certa** — veja [Escolhendo a placa certa](./velge-rett-plate) para o que combina com o seu filamento
