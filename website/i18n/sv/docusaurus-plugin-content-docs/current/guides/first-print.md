...
När utskriften är klar visar instrumentpanelen ett slutfört meddelande (och skickar avisering om du har ställt in [aviseringar](./notification-setup)).
...
- **Väg tomma spoler** — ange startvikt på spoler för exakt restberäkning (se [Filamentlager](./filament-setup))
- **Ställ in Telegram-aviseringar** — få besked när utskriften är klar utan att sitta och vänta (se [Aviseringar](./notification-setup))
- **Kontrollera byggplattan** — ren platta = bättre vidhäftning. Torka av med IPA (isopropanol) mellan utskrifter
- **Använd rätt platta** — se [Välja rätt byggplatta](./choosing-plate) för vad som passar ditt filament
