...
Baskı tamamlandığında, pano bir tamamlanma mesajı gösterir (ve [bildirimler](./varsler-oppsett) ayarladıysanız bildirim gönderir).
...
- **Boş makaraları tartın** — doğru kalan hesaplama için makaraların başlangıç ağırlığını girin (bkz. [Filament yönetimi](./filament-oppsett))
- **Telegram bildirimi kurun** — baskı bittiğinde beklemeden mesaj alın (bkz. [Bildirimler](./varsler-oppsett))
- **Yapı tablasını kontrol edin** — temiz tabla = daha iyi yapışma. Baskılar arasında IPA (izopropanol) ile temizleyin
- **Doğru plakayı kullanın** — filamentinize uygun olanı öğrenmek için [Doğru yapı plakasını seçmek](./velge-rett-plate) bölümüne bakın
