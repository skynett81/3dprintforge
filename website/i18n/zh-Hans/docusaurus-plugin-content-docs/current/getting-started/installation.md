...
sidebar_position: 1
title: 安装
description: 在服务器或本地计算机上安装 3DPrintForge
---
...
3DPrintForge 使用 `--experimental-sqlite`，该功能内置于 Node.js 22。不支持旧版本。
:::
...
:::info 自签名 SSL 证书
首次启动时，仪表板会自动生成自签名 SSL 证书。浏览器会显示警告——这是正常现象。请参阅[HTTPS 证书](./setup#https-sertifikater)了解如何安装自定义证书。
:::
...
完整配置请参阅 [Docker 部署](../advanced/docker)。
...
3DPrintForge 内置通过 GitHub Releases 自动更新的功能。您可以在仪表板的**设置 → 更新**中更新，或手动更新：
