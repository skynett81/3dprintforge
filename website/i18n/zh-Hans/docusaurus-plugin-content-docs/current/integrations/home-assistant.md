...
description: 通过 MQTT 自动发现、自动化实体和自动化示例将 3DPrintForge 与 Home Assistant 集成
---
...
- 3DPrintForge 和 Home Assistant 使用同一个 MQTT 代理
...
3DPrintForge 现在将为所有已注册的打印机发布自动发现消息。
...
:::info 注意
按钮（暂停/继续/停止）不通过 MQTT 自动发现发布。请使用 3DPrintForge API 从自动化中发送命令。
:::
