...
**3DPrintForge** 是一款适用于 Bambu Lab 3D 打印机的自托管全功能控制面板。它让您从单个浏览器标签页就能全面了解和控制打印机、耗材库存、打印历史记录等所有内容。

## 什么是 3DPrintForge？

3DPrintForge 通过局域网 MQTT 直接连接到您的打印机，无需依赖 Bambu Lab 的服务器。您也可以连接 Bambu Cloud 以同步模型和打印历史记录。
...
3DPrintForge 使用 Node.js 22 和原生 HTML/CSS/JS 构建 — 没有重型框架，没有构建步骤。数据库是 Node.js 22 内置的 SQLite。
